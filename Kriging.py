...
class Kriging():
    def __init__(self, corr_keyword, regr_keyword, y, x):
        self.corr_keyword = corr_keyword
        self.regr_keyword = regr_keyword
        self.x = np.copy(x)
        self.y = np.copy(y)

        self.regr_func_eval = Kriging.eval_regression_basis(regr_keyword)
        self.corr_func_scalar_eval, self.corr_func_vector_eval = Kriging.correlation_basis(corr_keyword)
    ...
    def info_eval(corr_vector, y, x, F, theta):
        ...
        return C_inv, R, beta, gamma, tilde_F
